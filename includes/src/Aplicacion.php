...
class Aplicacion
{
    ...
    public static function getunwanted_array(){
        return $this->unwanted_array;
    }
    ...
    private function doIncludeInterna($path, &$params, $retornaContenido=false)
    {
        $this->compruebaInstanciaInicializada();
        ...
        if($retornaContenido){
            $contenido=$contenido??"No ha pido cargar contenido";
            return $contenido;
        }
    }
    ...
    /**
     * Función que guarda una busqueda realizada en la aplicación
     */
    public function setBusqueda(Busqueda $search)
    {
        $this->compruebaInstanciaInicializada();
        $_SESSION['busqueda'] =  serialize($search);
    }

    public function getBusqueda()
    {
        $this->compruebaInstanciaInicializada();
        $search = unserialize($_SESSION['busqueda']??"");
        return $search;
    }

    public function delBusqueda()
    {
        $this->compruebaInstanciaInicializada();
        unset($_SESSION['Busqueda']);
        //Se destruye la sesión solo con el logout
    }

    public function setPiso(Piso $search)
    {
        $this->compruebaInstanciaInicializada();
        $_SESSION['piso'] =  serialize($search);
    }

    public function getPiso()
    {
        $this->compruebaInstanciaInicializada();
        $search = unserialize($_SESSION['piso']??"");
        return $search;
    }

    public function delPiso()
    {
        $this->compruebaInstanciaInicializada();
        unset($_SESSION['piso']);
        //Se destruye la sesión solo con el logout
    ...
    public function getHTML($path)
    {
        $this->compruebaInstanciaInicializada();
        $params = array();
        $aux = $this->doIncludeInterna($path, $params, true);

        return $aux;
    }

    // Una variable que los bloques de contenido pueden consultar para saber 
    // en qué pagina se encuentran y así generar el contenido adecuado

    //Reutilizamos esta variable para mostrar detalles piso??
    private $contexto; 
    ...
    public function comprueba_permisos($rol_necesario, $redirige='/permiso_denegado.php'){

        if($this->tieneRol($rol_necesario)){
            return true;
        }
        else{
            $this->redirige($this->resuelve($redirige));
        }
    }
